...
interface OrderDao {
    ...
    @Query("UPDATE orders SET rackCode = :rackCode, status = '${OrderStatus.DELIVERED_TO_WORKSHOP}', isSynced = 0, updatedAt = :updatedAt WHERE id = :orderId")
    suspend fun updateRackCode(orderId: String, rackCode: String, updatedAt: Long = System.currentTimeMillis())

    @Query("UPDATE orders SET paidAmount = :paidAmount, discountAmount = :discountAmount, paymentMethod = :paymentMethod, status = '${OrderStatus.DELIVERED_SETTLED}', isSynced = 0, updatedAt = :updatedAt WHERE id = :orderId")
    ...
    @Query("UPDATE orders SET status = '${OrderStatus.OFFICE_SETTLED}', isSynced = 0, updatedAt = :updatedAt WHERE status = '${OrderStatus.DELIVERED_SETTLED}'")
